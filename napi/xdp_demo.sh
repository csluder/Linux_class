...
# CRITICAL: Flush ARP caches before deleting namespaces
# Stale ARP entries can cause routing issues after recreation
log "Flushing ARP caches..."
ip netns exec client_ns ip neigh flush all 2>/dev/null || true
ip netns exec listener_ns ip neigh flush all 2>/dev/null || true
...
# IMPORTANT: Remove the entire directories to clear stale map data
rm -rf $BPF_PIN_DIR/xdp_cli \
       $BPF_PIN_DIR/xdp_drv \
       $BPF_PIN_DIR/xdp_lis \
       $BPF_PIN_DIR/veth_maps_cli \
       $BPF_PIN_DIR/veth_maps_lis \
       $BPF_PIN_DIR/drv_maps \
       $BPF_PIN_DIR/xdp_dummy 2>/dev/null || true
...
# Namespaces provide isolated network stacks for client and listener
...
# --- 8. Load XDP Programs ---
# Programs are loaded into the kernel and pinned to the BPF filesystem
...
# IMPORTANT: Get interface indices AFTER all interfaces are created and up
...
# Key 0 → Client veth(10.0.0.1) - use HOST-SIDE veth ifindex
bpftool map update pinned $BPF_PIN_DIR/drv_maps/route_map \
    key 0 0 0 0 value $CLI_IDX 0 0 0

# Key 1 → Listener veth(10.0.0.2) - use HOST-SIDE veth ifindex
bpftool map update pinned $BPF_PIN_DIR/drv_maps/route_map \
    key 1 0 0 0 value $LIS_IDX 0 0 0

# --- 11. Final ARP Cache Flush ---
# CRITICAL: Flush ARP caches after all configuration is complete
# This ensures no stale ARP entries from previous runs
log "Flushing ARP caches (final cleanup)..."
ip netns exec client_ns ip neigh flush all
ip netns exec listener_ns ip neigh flush all
...
echo "  Check:    sudo ./check_setup_timing.sh"
echo "========================================="
echo ""
echo "KNOWN ISSUE: Packet Queueing in l3loop0 Driver"
echo "  - Packets are queued and delivered in batches"
echo "  - This causes ~1 second latency"
echo "  - Root cause: 1ms timer delay accumulates"
echo "  - Solution: Modify driver to use immediate NAPI scheduling"
echo "            or reduce timer delay (requires careful testing)"
echo "========================================="
