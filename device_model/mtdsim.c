...
static int sim_nor_probe(struct platform_device *pdev) {
	...
	ret = device_create_file(&pdev->dev, &dev_attr_flash_size);
	if (ret) {
		mtd_device_unregister(&data->mtd);
		vfree(data->buffer);
		return ret;
	...
	dev_info(&pdev->dev, "Simulated MTD NOR (6.12+) Probed\n");
	return 0;
}
...
static void sim_nor_remove(struct platform_device *pdev) {
	...
	dev_info(&pdev->dev, "Simulated MTD NOR Removed\n");
}

static struct platform_driver sim_nor_driver = {
	.probe = sim_nor_probe,
	.remove = sim_nor_remove,
	.driver = { .name = "sim_nor", .owner = THIS_MODULE },
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Simulated MTD NOR Flash for Kernel 6.12+");
